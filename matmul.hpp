 ...
 * | 03 13 23 33 |  | 3x -> |  |             |
 */
void do_4x4(int lda, int K, double* a, double* b, double* c) 
{
    __m256d a0x_3x,
            bx0, bx1, bx2, bx3,
            c00_30, c01_31,
...
void copy_a4(int lda, const int K, double* a_src, double* a_dest) 
{
    for (int i = 0; i < K; ++i) {
        *a_dest++ = *a_src;
        *a_dest++ = *(a_src + 1);
...
void copy_b4(int lda, const int K, double* b_src, double* b_dest) 
{
...
template <class T, std::size_t MK, std::size_t KN, std::size_t MN>
void do_avx256(int lda, int M, int N, int K, std::array<T,MK>& A, std::array<T,KN>& B, std::array<T,MN>& C) 
{
    __m256d m0,m1,m2,m3;
    for (int i = 0; i < MK; i += 4) {
        for (int j = 0; j < KN; ++j) {
